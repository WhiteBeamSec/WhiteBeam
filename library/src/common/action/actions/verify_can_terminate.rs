        ...
        if (pid == service_pid) ||
           ...
           ((pid == -1) && (platform::get_current_uid() == 0)) {
            ...
            if !(crate::common::db::get_prevention()) {
                event::send_log_event(libc::LOG_NOTICE, format!("Detection: {} -> {} killed the WhiteBeam service (VerifyCanTerminate)", &par_prog, &src_prog));
                return (hook, args, do_return, return_value);
            }
            ...
            event::send_log_event(libc::LOG_WARNING, format!("Prevention: Blocked {} -> {} from killing WhiteBeam service (VerifyCanTerminate)", &par_prog, &src_prog));
