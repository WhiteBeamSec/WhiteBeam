...
whitebeam_test!("linux", initialization_ld_audit {
...
whitebeam_test!("linux", initialization_ld_bind_not {
...
whitebeam_test!("linux", initialization_env_sanity {
...
whitebeam_test!("linux", initialization_wb_parent {
...
whitebeam_test!("linux", initialization_wb_prog {
    let wb_prog_value = std::process::Command::new("/bin/bash").arg("-c").arg("/usr/bin/printenv WB_PROG").output().expect("printenv command failed to start");
    assert!(wb_prog_value.stdout.iter().eq(b"/usr/bin/printenv\n"));
});